...
**데이터 이용 주의**: 네이버 데이터랩 검색어트렌드 API로 받은 결과다. 이 저장소에는 학습 목적의 분석 재현을 위해 수집 결과(JSON·CSV)를 함께 두었다. 원본 데이터의 권리는 네이버에 있으며, API 이용약관에 따라 상업적 재배포·재판매는 하지 않는다. API 키는 저장소에 포함하지 않았다(`.env`는 `.gitignore` 처리, 견본만 `.env.example`로 제공).
...
### 7-1. 시계열 분해 (추세 · 계절성 · 나머지)

![통영 시계열 분해](images/05_통영_시계열분해.png)

통영 시계열을 세 조각으로 나눴다. 라이브러리를 쓰지 않고 다음 순서로 계산했다.

1. 추세 = 12주 중심 이동평균
2. 계절성 = (원본 − 추세)를 ISO 주차별로 평균
3. 나머지 = 원본 − 추세 − 계절성

- **검산**: 추세+계절성+나머지와 원본의 최대 차이 0.0
- **관찰**: 계절 요인만으로 31주차 +12.0, 35주차 −8.2로 20포인트 넘게 오르내린다. 추세는 2025년에 뚜렷이 낮아진다.
- **한계**: 계절성을 5년 평균 하나로 고정했다. 해마다 계절 패턴이 달라졌다면 그 변화는 나머지로 밀려난다.

### 7-2. 대시보드 (Streamlit, 로컬 실행)

`app.py`. 저장된 CSV만 읽고 API는 호출하지 않는다. 제출 방식은 **스크린샷 세트 + 필터 변경 시나리오**.

| 화면 | 조작 | 확인되는 것 |
|---|---|---|
| ![기본 화면](images/dashboard_01.png) | 기본값(3개 지역 · 2021~2025 · 주간 · 12주) | 남해 > 통영 > 거제 순서. 평균 25.0 / 11.6 / 33.1 |
| ![통영만](images/dashboard_02.png) | 지역에서 거제·남해 제거 | 통영만 남고 세로축이 다시 잡힌다. 요약 숫자도 함께 바뀐다 |
| ![기간 축소](images/dashboard_03.png) | 기간을 2024~2025로 축소 | 평균이 23.3 / 9.0 / 25.8로 바뀌고 2025년 하락 구간이 확대된다 |
| ![월별 평균](images/dashboard_04.png) | 아래로 스크롤 | 7월 정점 구조를 지역별 막대로 비교 |
